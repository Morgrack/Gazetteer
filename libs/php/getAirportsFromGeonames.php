...
$output = validate_cURL_JSON('http://api.geonames.org/searchJSON?country=' . $_REQUEST['countryCode'] . '&q=airport&maxRows=1000&username=' . $geoNamesUsername . '&style=full');
if ($output['data'] !== null) {
    if (count($output['data']['geonames']) > 0) {
        $newArray = [];
        foreach ($output['data']['geonames'] as $value) {
            if ($value['fcodeName'] === 'airport') {
                $object = new stdClass();
