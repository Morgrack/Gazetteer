...
if ($output['data'] !== null) {
    if (count($output['data']['results']) > 0 && $output['data']['status']['code'] === 200) {
        ...
        if (array_key_exists('ISO_3166-1_alpha-3', $output['data']['results'][0]['components'])) {
            $output['data'] = [$output['data']['results'][0]['components']['ISO_3166-1_alpha-2'], $output['data']['results'][0]['components']['ISO_3166-1_alpha-3']];
        } else {
            $output['data'] = 'ocean';
        }
    } else {
